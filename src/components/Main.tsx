...
const Main: React.FC = () => {
  ...
  const handlePrefectureChange = async (prefCode: number, checked: boolean) => {
    ...
    if (checked) {
      ...
      setPopulationData((prev) => prev.filter((item) => item.prefName !== prefectures.find((pref) => pref.prefCode === prefCode)?.prefName));
    }
  };
  ...
  return (
    <div className="container">
      <h1>都道府県別人口推移</h1>
      <PrefectureList prefectures={prefectures} onPrefectureChange={handlePrefectureChange} />
